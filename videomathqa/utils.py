...
with open(Path(__file__).parent / "videomath_mcq.yaml", "r") as f:
...
def videomath_doc_to_visual(doc):
...
def videomath_doc_to_text(doc, lmms_eval_specific_kwargs=None):
    if len(doc["options"]) == 2:
        option_prompt = "Select the best answer to the following multiple-choice question based on the video. Respond with only the letter (A or B) of the correct option."
    else:
...
def videomath_doc_to_text_subtitle(doc, lmms_eval_specific_kwargs=None):
    cache_dir = os.path.join(base_cache_dir, cache_name)
    video_path = doc["videoID"] + ".mp4"
    video_path = os.path.join(cache_dir, "videos", video_path)
    ...
    if len(doc["options"]) == 2:
        option_prompt = "Select the best answer to the following multiple-choice question based on the video and the subtitles. Respond with only the letter (A or B) of the correct option."
    else:
...
def videomath_process_results(doc, results):
    """
    Args:
        doc: a instance of the eval dataset
        results: [pred]
    Returns:
        a dictionary with key: metric name (in this case videomath score), value: metric value
    """
    pred = results[0]
    ...
    return {f"videomath_perception_score": data_dict}


def videomath_aggregate_results(results):
    """
    Args:
    ...
    for video_length in VIDEO_LENGTH:
        ...
        length_scores.append(f"{score:.2f}")
    ...
    for category in CATEGORIES:
        ...
        for k, v in category2score.items():
            if category in k:
                ...
                total_answered += v["answered"]
        score = 100 * total_correct / total_answered if total_answered > 0 else 0
        category_scores.append(f"{score:.2f}")
    ...
    combined_scores = [f"{overall_score:.2f}"] + category_scores + length_scores
    eval_logger.info("All Scores: \n" + ", ".join(combined_scores))

    return 100 * total_correct / total_answered if total_answered > 0 else 0


def multi_binary_aggregate_results(results):
