...
from videomath.utils import (extract_characters_regex,
                             multi_binary_aggregate_results,
                             videomath_aggregate_results,
                             videomath_process_results)
...
def postprocess_jsonl(llm, sampling_params, tokenizer, sample_jsonl, output_jsonl):
    ...
    for item in tqdm(updated_samples, desc="Computing scores..."):
        ...
        result = videomath_process_results(item["doc"], [pred_clean])
        processed.append(result["videomath_perception_score"])

    if mcq:
        final_score = videomath_aggregate_results(processed)
    else:
        final_score = multi_binary_aggregate_results(processed)
    print(f"Final Postprocessed VideoMath Score: {final_score:.2f}")
    print(f"Saved {len(updated_samples)} updated samples to {output_jsonl}")
